    ...
    <form action="${url.loginAction}" method="post">
      <div class="form-group"><label for="domain">Directory domain</label>
        <select id="domain" name="domain" class="form-control" autofocus>
          <option value="b">Domain B — b.consoto.com</option>
          <option value="c">Domain C — c.consoto.com</option>
        </select>
      </div>
      <div class="form-group"><label for="username">Directory user</label>
        <input id="username" name="username" class="form-control" value="PRV-PML" autocomplete="username" />
      </div>
